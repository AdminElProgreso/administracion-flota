...
const Team = () => {
  // Initial Data moved to state to allow mutations
  const [members, setMembers] = useState([
    { id: 1, name: 'Roberto Gomez', role: 'Jefe de Logística', dept: 'Logística', email: 'roberto.g@elprogreso.com', color: 'blue', img: 'https://lh3.googleusercontent.com/aida-public/AB6AXuD8PINEXI6Gb_LsuYADJm7CCLJnjPCatybEnz4-YWvXkj1WVNw-mx37kYWF_ForAuJpBBxdV4SxUqtVUTqRpTjxkp0ZlEdPuqYevLcw21GG68mHqn1vpAS7vVvOeFXilCgIPUGZ1tL60l-40FhR6oGjqD1MHCMNF1MXmO5L46NcGduT7kHfR2Wh72fLjLcA3ExYQAEGlmyoa2VhiI0XDa92zhUwkVFpBLRekEYzCHBNUdM7XT8CWTuBBgHrmYyjfzzYCRBa4yRFSz0', status: 'active' },
    { id: 2, name: 'Ana Martínez', role: 'Coordinadora de Pagos', dept: 'Administración', email: 'ana.m@elprogreso.com', color: 'purple', img: 'https://lh3.googleusercontent.com/aida-public/AB6AXuAxxyojVyjdfAU0_tSZZIZSWLGBGBHm4VJACJREJDtYXXNt8zw_eyqSh5xutOuRYfSc0Z98dxF0vVHvXRTldHpr3IYz7yyuUku_PCtGiQD2EtO7hEmcaTK6JbJ0FvXKzDd2mtCUt4HcYZ55dvizwaJJ5IVLo7yPjBnXkKBqpHs67H38Bm8VOnUvDrP6qMKf-AoBc7Jbv0RF31ZlD2pFpfsjbKHkBYrQJW5xk-1-1Sq8OLExrccDA5hdOKQpsRJt6Tgg2IAALw7EnZI', status: 'active' },
    { id: 3, name: 'Carlos Ruiz', role: 'Supervisor de Planta', dept: 'Cereales', email: 'carlos.r@elprogreso.com', color: 'amber', img: 'https://lh3.googleusercontent.com/aida-public/AB6AXuCmSpeJ2pUageUviIqNcq75yXvg3EKkH117Z_KZNVxSPRCvmy4IqKwKkj9uM11ysrX6jsKg1bNMgig13YoZMX5m-uLVm-Z03MQB4g1vHDM7BZwPkPP-TEjb_fsw3XnTZ-zngEPPQEM_YY2crrC1UayzavadRsp9k6qPPmxlyGbmuuyJHOSaSSXTj9MI2vlLagnwHyQgpvdNm1NwCE8RD-6oXEgv3s3f7bs31fprAhFx8rgeqirJLrCfjtf4WMCNBP957btgtKcc60E', status: 'inactive' },
    { id: 4, name: 'Miguel Angel', role: 'Mecánico Jefe', dept: 'Mantenimiento', email: 'miguel.a@elprogreso.com', color: 'orange', img: 'https://lh3.googleusercontent.com/aida-public/AB6AXuAY_RYd0-wx4b2AmvVMwtCayLJ_0utKRN3AAa0u8DZ6k1v-z-fvTgB2Tvu_5hTAE7EQFPPThRzpJc0jBQ_2LcTCPXCwTQcAVnleli4kOdVBhVcPP27KHJOqQhigJtlR8yvLoehrwYtdJxKGqJ9cslL7sdef5BTJ88rOa-afkGk8z2tFlqFxYvFxhs7KXt3l2p59AaIIOglpqcC4BdSkCN1uIpQKpfoF9Z2GQoxqnxefYjAuzfySPeQ0hrJwd2cCClLb5t2cBEhKZwE', status: 'active' },
  ]);

  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingMember, setEditingMember] = useState<any>(null); // null implies "Add Mode"
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    const newMemberData = {
      id: editingMember ? editingMember.id : Date.now(), // Keep ID if editing, new ID if creating
      name: formData.get('name') as string,
      role: formData.get('role') as string,
      email: formData.get('email') as string,
      dept: formData.get('dept') as string,
      status: formData.get('status') as string,
      color: 'blue', // Default color for simplicity in this demo
      img: editingMember ? editingMember.img : '', // Keep existing image or blank for new
    };

    if (editingMember) {
      // Update existing
      setMembers(members.map(m => m.id === editingMember.id ? { ...m, ...newMemberData, img: m.img } : m));
    } else {
      // Add new
      setMembers([...members, { ...newMemberData, img: '' }]);
    }
    ...
    <div className="p-6">
      <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-8">
        <div>
           ...
           <p className="text-stone-400 text-sm mt-1">Gestión de personal y preferencias de notificación.</p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
         {members.map((member) => (
            <div key={member.id} className={`group relative flex flex-col items-center p-6 rounded-xl bg-brand-surface border border-brand-border hover:border-primary/50 transition-all duration-300 hover:shadow-xl hover:shadow-black/20 ${member.status === 'inactive' ? 'opacity-75' : ''}`}>
               ...
               {/* Status Dot */}
               <div className="absolute top-4 left-4 flex gap-2" title={member.status === 'active' ? 'Activo' : 'Inactivo'}>
               ...
               <div className="w-full grid grid-cols-2 gap-3 mt-auto">
                  <button className="flex items-center justify-center gap-2 py-2 rounded-lg bg-stone-800 border border-brand-border hover:bg-stone-700 hover:text-white text-stone-400 text-xs font-medium transition-colors">
                     Llamar
                  </button>
                  <button className="flex items-center justify-center gap-2 py-2 rounded-lg bg-stone-800 border border-brand-border hover:bg-stone-700 hover:text-white text-stone-400 text-xs font-medium transition-colors">
                     Email
      ...
      {isModalOpen && (
         <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm" onClick={() => setIsModalOpen(false)}>
            <div className="relative w-full max-w-[600px] bg-brand-surface rounded-xl border border-brand-border shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
               ...
               <form onSubmit={handleSave} key={editingMember ? editingMember.id : 'new'}>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                           <div>
                              ...
                              <input 
                                ...
                                required
                                type="text" 
                                className="w-full bg-stone-800 border-none rounded-lg h-12 px-4 text-white focus:ring-2 focus:ring-primary" 
                  ...
                  <div className="p-6 border-t border-brand-border bg-brand-surface flex justify-between gap-4">
                     {editingMember && (
                        <button 
                           ...
                           className="h-12 px-6 border border-red-500/30 text-red-500 hover:bg-red-500/10 font-bold rounded-lg transition-colors flex items-center gap-2"
                        >
