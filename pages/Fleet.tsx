...
const Fleet = () => {
   ...
   return (
      <div className="p-4 md:p-6 pb-24 md:pb-6">
         ...
         {/* LISTADO (Mantenido igual para evitar cambios innecesarios) */}
         ...
         {/* --- MODAL DE ALTA RESTAURADO CON DISEÑO PROFESIONAL --- */}
         {isModalOpen && (
               ...
               <div className="relative w-full max-w-4xl bg-brand-surface rounded-xl border border-brand-border shadow-2xl flex flex-col max-h-[95vh]" onClick={e => e.stopPropagation()}>
                  <form onSubmit={handleAddVehicle} className="flex flex-col h-full">
                     ...
                     <div className="flex-1 overflow-y-auto p-8">
                        {/* Selector de Tipo (Añadido Tractor) */}
                           ...
                           <div className="space-y-6">
                              ...
                              <div className="grid grid-cols-2 gap-4">
                                 {newVehicleType !== 'generator' && (
                                    <div className="col-span-1"><label className="text-xs text-stone-400 mb-1 block">Patente <span className="text-red-500">*</span></label><input name="patente" required className="w-full bg-brand-dark border-brand-border rounded-lg text-white font-mono uppercase text-center h-10 border focus:border-primary focus:ring-0" placeholder="AA 000 BB" /></div>
                                 )}
                                 <div className={newVehicleType === 'generator' ? 'col-span-2' : 'col-span-1'}><label className="text-xs text-stone-400 mb-1 block">{newVehicleType === 'generator' || newVehicleType === 'tractor' ? 'Horas de Uso Actual' : 'Kilometraje Actual'}</label><input name="odometro" type="number" className="w-full bg-brand-dark border-brand-border rounded-lg text-white h-10 px-3 border focus:border-primary focus:ring-0" placeholder="0" /></div>
                                 <div className="col-span-1"><label className="text-xs text-stone-400 mb-1 block">Modelo</label><input name="modelo" required className="w-full bg-brand-dark border-brand-border rounded-lg h-10 px-3 text-white border focus:border-primary focus:ring-0" placeholder="Ej. Hilux SRV" /></div>
                                 <div className="col-span-1"><label className="text-xs text-stone-400 mb-1 block">Año</label><input name="anio" type="number" className="w-full bg-brand-dark border-brand-border rounded-lg h-10 px-3 text-white border focus:border-primary focus:ring-0" placeholder="2024" /></div>
                              </div>
                              <div className="bg-brand-dark/30 p-4 rounded-lg border border-brand-border">
                                 <div className="mb-3"><label className="text-xs text-stone-400 mb-1 block">Sección Operativa <span className="text-red-500">*</span></label><select name="seccion" className="w-full bg-brand-surface border-brand-border rounded-lg text-white h-10 px-3 border focus:border-primary focus:ring-0"><option>Administración</option><option>Cereales</option><option>Agronomía</option><option>Hacienda</option><option>Estación de Servicio</option><option>Miel</option><option>Veterinaria</option><option>Ferreteria</option><option>Supermercado</option><option>Balanceado</option></select></div>
                                 <div className="mb-3"><label className="text-xs text-stone-400 mb-1 block">Encargado de Sección <span className="text-red-500">*</span></label><input name="encargado" className="w-full bg-brand-surface border-brand-border rounded-lg text-white h-10 px-3 border focus:border-primary focus:ring-0" placeholder="Nombre" /></div>
                                 {newVehicleType !== 'generator' && (<div><label className="text-xs text-stone-400 mb-1 block">Chofer Asignado (Opcional)</label><input name="chofer" className="w-full bg-brand-surface border-brand-border rounded-lg text-white h-10 px-3 border focus:border-primary focus:ring-0" /></div>)}
                              </div>
                           </div>
